...
class Operator:
    ...
    def apply(self, state):
        ...
        @return A new state (set of predicates) after the application of the
                operator
        """
        return frozenset((state - self.del_effects) | self.add_effects) # need of frozenset through because state was modified and delete effects first, before adding the predicates that bevome true.
...
class Task:
    ...
    def get_successor_states(self, state):
        ...
        for operator in self.operators: # for every operator of Task Instance
            if operator.applicable(state):  # is operator applicable?
                new_state = operator.apply(state)  # apply to get new state
                successor_states.append((operator, new_state))  # store the (op, new_state) in a list
